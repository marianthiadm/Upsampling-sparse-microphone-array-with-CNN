...
class cov_upsam(nn.Module):
    def __init__(self):
        ...
        self.up1 = FreqDependentConv1D(in_channels=32, out_channels=64,kernel_size=3,stride=1,padding=1)
        self.norm1 = ChannelLayerNorm(64)
        # First upsampling (×2)
        self.up2 = FreqDependentConv1D(in_channels=64, out_channels=128, kernel_size=3,stride=1,padding=1) # batch, channel, f, t
        self.norm2 = ChannelLayerNorm(128)
        # Second upsampling (×2 again, total ×4)
        self.up3 = FreqDependentConv1D(in_channels=128, out_channels=256,kernel_size=3,stride=1,padding=1)
        self.norm3 = ChannelLayerNorm(256)
        # upsampling (×2)
        self.up4 = FreqDependentConv1D(in_channels=256, out_channels=512,kernel_size=3,stride=1,padding=1)
        self.norm4 = ChannelLayerNorm(512)
        # upsampling (×2)
        self.up5 = FreqDependentConv1D(in_channels=512, out_channels=1024,kernel_size=3,stride=1,padding=1)
